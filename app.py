...
def clean_dataframe(df):
    ...
    # 2. Strip whitespace from all string values in the dataframe
    df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
    
    # 3. Fill NaNs
    df = df.fillna("")
    
    # 4. Ensure S.NO is numeric
...
def get_initial_dataset():
    # Full dataset hardcoded for persistence
    data = {
        "S.NO": range(1, 216),
    ...
    # Create DataFrame robustly (handles unequal list lengths by padding with NaN)
...
with tab_cards:
    if not df_display.empty:
        ...
        for index, row in df_display.iterrows():
            ...
            # Call Button Logic - Fixed Layout
            ...
            # HTML Card Structure - Matching User Snippet Exactly
            card = f"""
